...
- `openpyxl` v3.0.10 or hinger
- `pyfastx` v2.3.0 or hinger
- `pyhmmer` v0.12.0
- `pyrodigal-gv` v0.3.2
...
After you got the result from `findGEVE`, you can generate a visualization for each retained GEVE using `findGEVE_plot.py`. Two output file, which are `.markerout` and `.geve.bed` are designed for visualization. Since findGEVE v0.5.0, this visualization output will be generated by default, so most of case no need to run it separately.
```
