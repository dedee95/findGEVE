#!/usr/bin/env python3
# Usage: python extract_tir.py input.geve.fna
...
def main():
    if len(sys.argv) < 2:
        print("Usage: extract_tir.py input.geve.fna")
        sys.exit(1)
    ...
    with open(out_path, "w") as fh:
        for header, seq in records:
            ...
            combined  = tir_l + revcomp(tir_r)
            geve_name = get_geve_name(header)
            write_fasta(fh, geve_name, combined)
            n_written += 1

    print(f"Done: {n_written} TIR sequence(s) written, {n_skipped} no-TIR skipped")
